...
docker compose --project-directory "$repository_root" --env-file "$runtime_env" \
  ...
  node -e "fetch('http://127.0.0.1:3001/v1/healthz',{headers:{Authorization:'Bearer '+process.env.PRIVATE_API_SERVICE_CREDENTIAL}}).then(r=>{if(!r.ok)throw new Error('health status '+r.status);console.log('Private API healthy')})"
