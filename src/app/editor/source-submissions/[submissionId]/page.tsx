...
export default async function SourceSubmissionReviewRoute({ params }: Readonly<{ params: Promise<{ submissionId: string }> }>) { const { submissionId } = await params; const editor = await editorSession(); const state = await loadSourceSubmissionReview(editor, submissionId); if (state.kind === "not-found") notFound(); if (state.kind === "unavailable") return <p className="editor-route-unavailable">The preparation review service is unavailable. No proposal was loaded.</p>; const acceptAction=state.submission.preparedOutputFingerprint?acceptProposalAction.bind(null,submissionId,state.submission.preparedOutputFingerprint):undefined;const sourceAction=state.submission.preparedOutputFingerprint?acceptSourceAction.bind(null,submissionId,state.submission.preparedOutputFingerprint):undefined; return <SourceSubmissionReviewPageContent editor={editor} submission={state.submission} acceptAction={acceptAction} acceptSourceAction={sourceAction} />; }
